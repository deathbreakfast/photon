...
**Getting started (topology):** [Embedded](https://docs.rs/uf-photon/latest/photon/#embedded-one-binary) · [Brokered](https://docs.rs/uf-photon/latest/photon/#brokered-publisher--worker-binaries)
...
| `PHOTON_BENCH_CRYPTO` | Unset (encrypt on) | Bench-only: `0`/`false` stores plaintext envelopes to measure crypto cost. BM-PD0/PD1/PD2/PD3 reject `0` and fail the report closed. |
...
| `PHOTON_BENCH_CRYPTO` | `0` disables crypto in bench runs. BM-PD0/PD1/PD2/PD3 reject this and fail the report closed. |
| `PHOTON_BENCH_OFFERED_RATE` | Offered publish rate for BM-PD2/PD3. Omit for an in-process sweep (`PHOTON_BENCH_OFFERED_RATE_SWEEP`, default `50,100,250,500,1000`). |
