#!/usr/bin/env bash
# Re-run PFH sweep + rsync (skip provision/build when bench host is ready).
set -euo pipefail
...
# Sync updated scripts + bench binary source if needed
rsync -az -e "ssh ${SSH_OPTS[*]}" \
  "$REPO/infra/aws/broker-fleet/scripts/" \
  "${SSH_USER}@${BENCH_PUBLIC_IP}:${REMOTE_DIR}/infra/aws/broker-fleet/scripts/"
rsync -az -e "ssh ${SSH_OPTS[*]}" \
  "$REPO/photon-bench/" "${SSH_USER}@${BENCH_PUBLIC_IP}:${REMOTE_DIR}/photon-bench/"

ssh "${SSH_OPTS[@]}" "${SSH_USER}@${BENCH_PUBLIC_IP}" \
  "test -x ${BENCH_BIN} || (source \$HOME/.cargo/env 2>/dev/null; export CARGO_TARGET_DIR=/tmp/photon-target CARGO_INCREMENTAL=0 CARGO_BUILD_JOBS=2 && cd ${REMOTE_DIR} && cargo build --release -p photon-bench --features nats)"

install_nats='if command -v nats >/dev/null 2>&1; then exit 0; fi
  ver=0.1.5
  arch=$(uname -m); case "$arch" in x86_64) arch=amd64;; aarch64) arch=arm64;; esac
  curl -sfL -o /tmp/nats.zip "https://github.com/nats-io/natscli/releases/download/v${ver}/nats-${ver}-linux-${arch}.zip" || exit 0
...
for host in "$BROKER_SINGLE_IP" "$BROKER_1_IP"; do
  ssh "${SSH_OPTS[@]}" "${SSH_USER}@${host}" "$install_nats" || true
done
