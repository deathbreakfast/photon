#!/usr/bin/env bash
# Bootstrap 4-node Kafka KRaft cluster on all broker EC2 hosts.
set -euo pipefail
...
QUORUM="1@${BROKER_1_IP}:9093,2@${BROKER_2_IP}:9093,3@${BROKER_3_IP}:9093,4@${BROKER_SINGLE_IP}:9093"
...
remote_bootstrap "$BROKER_SINGLE_PUBLIC_IP" cluster 4 "$BROKER_SINGLE_IP" "$QUORUM"
