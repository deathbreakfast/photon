...
TEARDOWN_BENCH="${TEARDOWN_BENCH:-0}"
if [[ "$TEARDOWN_BENCH" != "1" ]]; then
  echo "TEARDOWN_BENCH=0: terminating broker instances only (keeping bench hosts)"
  IDS=()
  [[ -n "${INSTANCE_BROKER_SINGLE:-}" ]] && IDS+=("$INSTANCE_BROKER_SINGLE")
  [[ -n "${INSTANCE_BROKER_1:-}" ]] && IDS+=("$INSTANCE_BROKER_1")
  [[ -n "${INSTANCE_BROKER_2:-}" ]] && IDS+=("$INSTANCE_BROKER_2")
...
if [[ -n "${SECURITY_GROUP_ID:-}" && "$TEARDOWN_BENCH" == "1" ]]; then
  sleep 5
