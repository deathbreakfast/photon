#!/usr/bin/env bash
# Bootstrap 4-SPU Fluvio cluster on all broker EC2 hosts.
set -euo pipefail
...
clean_host() {
  ...
  ssh "${SSH_OPTS[@]}" "${SSH_USER}@${host}" \
    "sudo docker rm -f \$(sudo docker ps -aq --filter name=photon-fluvio) 2>/dev/null || true"
}

for h in "$BROKER_SINGLE_PUBLIC_IP" "$BROKER_1_PUBLIC_IP" "$BROKER_2_PUBLIC_IP" "$BROKER_3_PUBLIC_IP"; do
  clean_host "$h"
done
...
remote_bootstrap "$BROKER_SINGLE_PUBLIC_IP" spu 5001 "$BROKER_SINGLE_IP" "$SC_IP" 9110 9111
remote_bootstrap "$BROKER_1_PUBLIC_IP" spu 5002 "$BROKER_1_IP" "$SC_IP" 9110 9111
remote_bootstrap "$BROKER_2_PUBLIC_IP" spu 5003 "$BROKER_2_IP" "$SC_IP" 9110 9111
remote_bootstrap "$BROKER_3_PUBLIC_IP" spu 5004 "$BROKER_3_IP" "$SC_IP" 9110 9111
