...
pub fn project_fleet(hardware: &str, storage: &str, reports_dir: &Path, out: Option<PathBuf>) -> Result<()> {
    ...
    if storage == "nats" {
        ...
        if let Ok(curve) = curve {
            projection.broker_nodes_for_1e9 = curve.broker_nodes_for_target.get("1000000000").copied();
    ...
    let out_path = out.unwrap_or_else(|| {
        reports_dir.join(format!("projection-{hardware}-{storage}.json"))
    });
